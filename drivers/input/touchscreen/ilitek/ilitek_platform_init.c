...
static int /*__devinit*/ tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = ilitek_main_probe(client, id);
...
{
	return ilitek_main_probe(client, id);
}
...
static int __init ilitek_touch_driver_init(void)
{
	...
	if (ret) {
		tp_err("add touch device driver i2c driver failed.so remove\n");
		i2c_del_driver(&ilitek_touch_device_driver);
		return -ENODEV;
	}
	tp_msg("add touch device driver i2c driver.\n");
	return ret;
}
