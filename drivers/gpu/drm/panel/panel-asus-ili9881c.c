...
static int ili9881c_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	*bus_flags |= DRM_BUS_FLAG_DE_LOW | DRM_BUS_FLAG_PIXDATA_SAMPLE_POSEDGE;
	ret = drm_display_info_set_bus_formats(&connector->display_info,
					       &bus_format, 1);
	if (ret){
		printk("Failed to set  bus formats, ret %d!\n", ret);
		return ret;
	}
...
{
	struct ili9881c *ctx = container_of(work, struct ili9881c, backlight_work.work);
...
int ili9881c_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	INIT_DELAYED_WORK(&ctx->backlight_work, backlight_enable_work);
